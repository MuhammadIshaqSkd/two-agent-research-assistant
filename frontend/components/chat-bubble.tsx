  ...
  if (message.role === "user") {
    return (
      <div className="flex justify-end animate-fade-up">
        <div className="max-w-[85%] sm:max-w-[75%] rounded-2xl rounded-tr-sm bg-panel-2 px-3.5 py-2 text-[13.5px] text-foreground ring-1 ring-line">
          {message.content}
        </div>
      </div>
    );
    ...
    <div className="flex gap-3 animate-fade-up">
      ...
      <div className="min-w-0 flex-1 space-y-2">
        <div className="flex items-baseline gap-2 text-[11px]">
          ...
          {message.streaming && <span className="text-warn">streaming</span>}
        </div>
        {message.tool && (
          <ToolCallCard
